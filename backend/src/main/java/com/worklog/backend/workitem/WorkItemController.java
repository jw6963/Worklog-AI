...
public class WorkItemController {
    ...
    public SearchResponse search(@RequestParam LocalDate from, @RequestParam LocalDate to,
        ...
        List<LocalDate> matchingDates = repository.findSearchDates(ownerId, from, to, beforeDate, type, projectId,
                normalizedQuery, PageRequest.of(0, limit + 1));
        ...
        List<WorkItem> items = pageDates.isEmpty()
                ? List.of()
                : repository.findSearchItems(ownerId, pageDates, type, projectId, normalizedQuery);
        LocalDate nextBeforeDate = hasMore ? pageDates.getLast() : null;
        return new SearchResponse(items, hasMore, nextBeforeDate,
                repository.countSearchItems(ownerId, from, to, type, projectId, normalizedQuery),
                repository.countSearchDates(ownerId, from, to, type, projectId, normalizedQuery));
    }
    ...
    public List<WorkItem> carryOver(@Valid @RequestBody CarryOverRequest request, Authentication auth) {
        ...
        List<WorkItem> copies = source.stream()
                .filter(item -> !existingContents.contains(item.getContent()))
                .map(item -> new WorkItem(request.toDate(), WorkItem.ItemType.TODO, item.getContent(), item.getProject(), owner))
                .toList();
    ...
    public BackupResponse backup(Authentication auth) {
        ...
        List<BackupProject> projects = projectRepository.findByOwnerIdOrderByArchivedAscNameAsc(owner.getId()).stream()
                ...
                .toList();
        return new BackupResponse(2, Instant.now(), projects, items);
    }
    ...
    public List<WorkItem> restore(@Valid @RequestBody RestoreRequest request, Authentication auth) {
        ...
        List<WorkItem> restored = request.items().stream()
                .map(item -> new WorkItem(item.workDate(), item.type(), item.content().trim(),
    ...
    public void delete(@PathVariable Long id, Authentication auth) {
        repository.delete(ownedItem(id, auth));
    }
